...
class Atoa extends CI_Controller
{
    ...
    public function hotelDetail($id, $title)
    {
        $data = array();
        $data['name'] = "Hotels";
        $data['main_back_link'] = 'directory-hotel-in-cambodia.html';
        ...
        $region_name = $data['hotel']->region_name;
        $region_name = str_replace('&', null, $region_name);
        $region_name = str_replace(' ', '-', $region_name);
        $region_name = str_replace('--', '-', $region_name);
        $region_name = strtolower($region_name);

        $data['back_link'] = 'directory-hotel-in-cambodia.html/' . $data['hotel']->region_id . '/' . $region_name;
        $data['back_link_text'] = $data['hotel']->region_name;

//        echo "<pre>" . print_r($data["hotel"], 1) . "</pre>";
//        exit;
        $data['current_page'] = $data['hotel']->name;
